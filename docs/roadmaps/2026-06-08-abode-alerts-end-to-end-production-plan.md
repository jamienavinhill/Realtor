...
- [!] Verify Firebase authorized domains include the production Vercel URL and the local development domain. **Operator-pending** — code ready (`scripts/add-auth-domains.ts` now reconciles `localhost`, `127.0.0.1`, `abode-alerts.vercel.app`, additive, extra domains via args/`AUTH_DOMAINS`); needs a live run with admin credentials.
- [!] Add budget alerts if Blaze or paid Google Cloud is enabled. **Operator-pending** — currently within free quotas; set in the Google Cloud console before enabling any billable service.
- [!] Redeploy production; read back the `44224` baseline (~88 listings) on the production URL. **Operator-pending** — `scripts/vercel-listings-check.ts` now smokes the live page AND the protected-route gate; needs a live run against the deployed URL.
...
| (dispatching) | WS2 pass 1 (re-run) | `lib/firebase-admin.ts`, `lib/env.ts` (admin-source only), `.env.example`, ingest-route token gating, `docs/operations/env-and-deploy.md`, operator scripts | 2026-06-09 | dispatching                                                                                                                                                                                                                                       | Gate; record operator actions, do not run live secret ops |
