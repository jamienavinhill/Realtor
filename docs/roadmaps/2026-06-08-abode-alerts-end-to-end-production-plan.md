...
- **Not started — remaining queue:**
  ...
  3. ~~**WS17** — tests/CI/release gate + full production smoke (final gate).~~ CLOSED `WS17_SHA` (pass 1): CI workflow + Playwright e2e suite (6 reachable smokes pass, 2 OAuth-gated documented-skip) + release-gate ladder + production runbook; `verify` GREEN, `test:rules` 42/42, `test:e2e` 6/8 (2 skip). Operator/manual residue: first CI run on push + the live OAuth-gated / deployed-only smoke rungs.
