...
| WS17 tests/CI/release gate + production smoke                             | CLOSED `eb68b067` (pass 1). Landed: CI workflow `.github/workflows/ci.yml` (`verify` + `rules` [Temurin JDK 21 + cached Firestore emulator] + `e2e` [Playwright chromium] jobs on push/PR to `main`; pinned action versions verified vs official docs; no real secrets — public Firebase web config builds clean). Playwright suite (`playwright.config.ts` + `e2e/` + `test:e2e`): 6 reachable smokes RUN+PASS — auth chrome signed-out, docs TOC pinned, CMA pagination/sort (public `properties` catalog), toast non-shift (shipped `abode:toast` event), protected route 401-without / non-401-with token; 2 OAuth-gated specs (listings dialog-action + WS18 share/invite) are documented `test.skip` → manual runbook, NOT faked, no auth backdoor. Release gate defined as a 10-rung ladder (`development-workflow.md`) stating CI vs operator/manual; production smoke runbook (`release-runbook.md`) maps to the Acceptance Criteria incl. the WS18 two-account flow + operator prerequisites. `npm run verify` GREEN (202 unit + lint/type/format/build), `npm run test:rules` 42/42 GREEN, `npm run test:e2e` 6 PASS / 2 skip. **Operator/manual residue:** first CI run on push; deployed protected-route smoke (`vercel-listings-check.ts`); live OAuth-gated UI + two-account sharing; Gmail watch/Pub-Sub + OAuth consent + authorized-domains + Vercel env readback + budget alert. |
...
- **Not started — remaining queue:**
  ...
  3. ~~**WS17** — tests/CI/release gate + full production smoke (final gate).~~ CLOSED `eb68b067` (pass 1): CI workflow + Playwright e2e suite (6 reachable smokes pass, 2 OAuth-gated documented-skip) + release-gate ladder + production runbook; `verify` GREEN, `test:rules` 42/42, `test:e2e` 6/8 (2 skip). Operator/manual residue: first CI run on push + the live OAuth-gated / deployed-only smoke rungs.
