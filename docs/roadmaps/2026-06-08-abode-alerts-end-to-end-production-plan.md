...
Date: 2026-06-08
Status: [x] Code-complete (all workstreams WS1–WS19 closed across two passes each; 2026-06-10). The only remaining work is operator/account-console actions that cannot be done in code — first CI run on push, the live OAuth-gated + deployed-only smoke rungs, Gmail watch/Pub-Sub + OAuth consent + authorized-domains + Vercel env readback + GCP budget alert. See the Orchestrator Checkpoints residue notes.
...
- [x] Every visible action has an end-to-end data path or is removed until it does. Scan Gmail (`parse_gmail`), direct parse (`parse_raw_text`), commit (`POST /api/properties` `action: commit` → Admin SDK upsert, WS16), delete listing (`action: delete_listing`, WS16), export (`export_sheets`), schedule (`create_calendar_event`), analyze (`/api/listings/analyze`), and the wizard guide (`/api/gemini`) are all wired to real routes. The new WS18 share/invite/accept/revoke/role-change actions hit `/api/account/{members,invite,accept,revoke}`; the workspace switcher re-targets per-user listeners by active owner uid.

Close-out audit (2026-06-10): WS15's bar was re-verified across the post-WS18/WS16 surfaces (ShareWorkspaceDialog, header workspace switcher, `/invite/[token]` accept page, and the server-moved `commit`/`delete_listing`/`parse_*` flows). Findings: metadata is final (`app/layout.tsx`, `config/app/metadata.json`, `app/icon.svg`, `app/opengraph-image.tsx` — Abode Alerts + `abode-alerts.vercel.app`; the invite page has no `generateMetadata`, so it inherits the root default title and never leaks the token); no fake data, stock media, or obsolete AI-Studio/Realty-Monitor/Austin strings in any shipped path (`rg` sweep clean; remaining hits are honest CMA "no simulated data" copy, HTML `placeholder` attributes, `tests/` fixtures, and the non-shipped `docs/research/INBOX_PARSING.md`); honest role states present (viewer read-only notes in ShareWorkspaceDialog / ListingsGrid modal / CMA dialog / Alerts create form; invite accepted/revoked/already-accepted/email-mismatch/error; switcher only renders when the user belongs to >1 workspace). Audit-only — no code change required. The shared-catalog `delete listing` button is correctly gated by auth (any signed-in user) rather than workspace role, since `properties` is a global catalog, not workspace-scoped.

Suggested verification:

- `npm run lint`, `npm run typecheck`; browser smoke across every tab. (Close-out 2026-06-10: `npm run lint`, `npm run typecheck`, `npm run format:check`, `npm run build`, `npm run verify` all GREEN — 202/202 unit tests. Browser smoke remains operator/CI-pending because the dashboard is auth-gated and Google OAuth is unavailable in this environment; the wired surfaces were verified by read-back + build.)
...
- [x] Audit `config/firebase/firestore.rules` for listings read access, user alert ownership, alert-match ownership, listing-preference own-only access, and admin/operator writes. Full first-principles audit done (incl. WS18 member rules); every invariant re-derived and backed by emulator cases. No hole found beyond the documented `properties` client-write gap (closed below). Pass 2: a fresh-eyes adversarial re-derivation again found no hole and added 4 explicit server-only deny proofs (`alert_matches`, `ingest_runs`, `provider_quota` reject signed-in + anon client writes/reads) — 42/42 emulator cases GREEN.
...
- [x] Gate the Gemini-extraction actions behind Firebase ID-token auth (defense-in-depth / cost protection). Pass 2: `parse_gmail` and `parse_raw_text` on `POST /api/properties` now require a verified Firebase ID token (`parse_raw_text` via `Authorization: Bearer`; `parse_gmail` via a separate `X-Firebase-Id-Token` header so its `Authorization` can still carry the Google OAuth access token for the Gmail REST calls). Unauthenticated callers get an honest 401 before any Gemini client is constructed. Route logic extracted to `lib/api/properties-handler.ts` (`handlePropertiesPost(req, deps)`); `components/dashboard.tsx` sends the ID token on both calls; Gmail/paste UX unchanged. Covered by `tests/properties-route.test.ts` (auth-required, invalid-token, commit happy path, delete, extraction gates) inside `npm run test`.
...
- [x] Users cannot write arbitrary shared listing state from the browser unless explicitly allowed. `properties` writes are `if false` for clients; all catalog writes go through the Admin SDK, and (pass 2) the upstream `parse_gmail`/`parse_raw_text` extraction actions require a verified ID token too. Proven by emulator cases (signed-in/anon create/update/delete denied; public read allowed) + the route integration test (`tests/properties-route.test.ts`).
- [x] Users can read/write only their own (or their workspace's, per WS18 role) alert, alert-match, and preference docs. Re-audited (pass 2, fresh eyes); viewer read-only, editor cannot re-owner/remove-owner; `alert_matches`/`ingest_runs`/`provider_quota` server-only with explicit deny proofs. 42 emulator cases (24 WS18) stay green.
...
- [x] WS1–WS16 (+ WS18, WS19). All closed and verified at HEAD.

Enables:

- [x] Reliable production operation and future agent handoffs.

Primary areas:

- `package.json`, `tests/` or framework-native test paths, `scripts/*`, `vercel.json`, `.github/workflows/*`, `docs/operations/development-workflow.md`, `playwright.config.ts`, `e2e/`, `docs/operations/release-runbook.md`

Implementation tasks:

- [x] Add a test harness appropriate for Next/React logic and provider adapters. (Node test runner over `tests/*.test.ts`, 202 unit/contract tests; `@firebase/rules-unit-testing` emulator suite under `tests/emulator/`.)
- [x] Add sanitized fixtures for RealtyAPI, Gmail extraction, and search enrichment. (`tests/fixtures/`, exercised by the provider/email/search tests.)
- [x] Add schema/adapter/repository tests and a query-builder unit test. (`schemas.test.ts`, `realty-normalize.test.ts`, `account-members-repo.test.ts`, `gmail-query-builder.test.ts`, et al.)
- [x] Add Playwright critical-path smokes: auth chrome, listings/compact dialog + one listing action, toast non-shift, docs TOC pinned, CMA pagination. (`playwright.config.ts` + `e2e/` + `test:e2e`. Reachable smokes — auth chrome (signed-out), docs TOC pinned, CMA pagination/sort on the public catalog, toast non-shift via the shipped `abode:toast` event, protected route 401/non-401 — RUN and PASS. Listings/dialog-action and WS18 share/invite require live Google OAuth, so they are documented `test.skip` in `e2e/auth-gated.spec.ts` → manual runbook; NOT faked, no auth backdoor.)
- [x] Add a CI workflow when Git is initialized. (`.github/workflows/ci.yml`: `verify` + `rules` (Java + Firestore emulator) + `e2e` jobs on push/PR to `main`; no real secrets; pinned action versions verified vs official docs.)
- [x] Define the release gate: lint, typecheck, format check, tests, build, env verification, protected-route smoke, Firestore rules verification. (Full 10-rung ladder in `docs/operations/development-workflow.md`, stating CI vs operator/manual.)
- [x] Production runbook: sign-in → profile-menu sign-out; ~88 listings with compact dialog + one action; toast on alert match without layout shift; docs TOC pinned; CMA pagination; protected ingest route 401/authorized; WS18 two-account flow. (`docs/operations/release-runbook.md`, mapped to Acceptance Criteria, with operator prerequisites.)

Exit criteria:

- [x] `npm run verify` is the reliable local release gate. (GREEN: lint, typecheck, format:check, 202 tests, build.)
- [x] CI runs equivalent gates once a GitHub repo exists. (`.github/workflows/ci.yml` runs `verify` + `test:rules` + `test:e2e` on push/PR to `main`; YAML linted vs official Actions docs. CI execution on GitHub is operator-pending the first push.)
- [x] The production smoke checklist is documented, repeatable, and passes — for the reachable surface. (`release-runbook.md` documented + repeatable; reachable e2e smokes pass; the OAuth-gated + deployed-only rungs stay operator/manual by design.)

Suggested verification:

- `npm run verify` (GREEN); `npm run test:rules` (42/42 GREEN); `npm run test:e2e` (6 reachable PASS, 2 OAuth-gated documented-skip); CI run on the first push.
...
_Status 2026-06-10 (pass 2): member data-read targeting is now wired — the dashboard routes its `alerts` / `alert_matches` listeners and `listingPreferences` / `compareQueue` reads+writes through the ACTIVE workspace owner uid (`lib/account/active-workspace.ts`, `useListingPreferences({ ownerUid, canWrite })`), so a member viewing an owner's workspace truly sees the owner's listings/alerts/CMA/preferences and an editor edits them (owner-pinned writes); a viewer is read-only (mutating controls hidden, denied by rules). Adversarial rules audit added pass-2 emulator cases (owner-as-member pollution, viewer alert/pref deletes, stranger writes, cross-owner pref forgery) and hardened the members rule (`memberUid != ownerUid`). `npm run verify` GREEN (180 unit tests + lint/type/format/build), `npm run test:rules` GREEN (32/32 incl. 24 WS18). Two-account live browser flow remains operator-pending (needs the Gmail watch/Pub-Sub + OAuth consent that also gate any shared user's account being live)._
...
19. [x] **WS15** — Wire all views/metadata to final Abode Alerts copy and data. _Pass 1 landed (`db669711`); pass-2 close-out audit (2026-06-10) over the post-WS18/WS16 surfaces found WS15 clean — metadata final, no fake data/obsolete copy, honest empty/role/error states, every action wired to a real route. Audit-only, no code change; `npm run verify` GREEN._
...
21. [x] **WS16** — Harden auth/security rules, OAuth token persistence, sharing rules, and production envs. _Code/docs closed across two passes: full rules audit, `properties` server-only writes + server commit/delete routes, OAuth-token-persistence confirmation, `auth-and-secrets.md`, App Check documented path (pass 1); ID-token gating of the `parse_gmail`/`parse_raw_text` extraction actions, `tests/properties-route.test.ts` route integration test, and a fresh-eyes adversarial rules re-audit with 4 added server-only deny proofs (pass 2). 42/42 rules + 202 unit GREEN. The only residue is operator-pending **account/console** actions (authorized-domain confirmation, Vercel env readback, GCP budget alert, App Check enablement) — no further code is required._
...
23. [x] **WS17** — Tests/CI/release gate and complete production smoke. _CLOSED across two passes (`eb68b067`,`21cc24e9` + pass-2 audit `__PASS2_SHA__`): CI workflow, Playwright e2e (6 reachable pass / 2 OAuth-gated documented-skip), 10-rung release-gate ladder + production runbook; pass-2 re-audited the CI YAML vs the GitHub release API (bumped `cache@v4`→`@v5`), tightened nothing weakened, and added an honest in-app Docs "Sharing a workspace" section. `verify` GREEN, `test:rules` 42/42, `test:e2e` 6 pass / 2 skip. Code lane closed; operator residue = first CI run on push + the live OAuth-gated/deployed-only smoke rungs._
24. [ ] Promote lasting rules to durable docs and retire the two superseded roadmaps. _Operator/maintenance follow-up; not part of the code lane._
...
| WS15 product flows / metadata / page wiring                               | CLOSED — pass 1 landed (`db669711`); pass-2 close-out audit (2026-06-10) over post-WS18/WS16 surfaces (ShareWorkspaceDialog, workspace switcher, `/invite/[token]`, server-moved `commit`/`delete_listing`/`parse_*`) found WS15 genuinely clean: final Abode Alerts metadata + `abode-alerts.vercel.app` (invite page inherits root title, no token leak); no fake data / obsolete brand strings in shipped paths; honest loading/empty/partial/provider-error/auth-required/no-media + new role states (viewer read-only notes in ShareWorkspaceDialog / ListingsGrid modal / CMA dialog / Alerts form; invite accepted/revoked/expired/error; switcher shows only when >1 workspace); every visible action (scan/paste/commit/delete/export/schedule/analyze/wizard + share/invite/accept/revoke/role-change/workspace-switch) maps to a real `/api/*` route. Audit-only, no code change required; `npm run verify` GREEN (202 unit + lint/type/format/build).                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| WS16 auth/rules/secret hardening                                          | CLOSED `155962d4`,`441eb3f5` (pass 1) + `15cf8068` (pass 2). Pass 1: full first-principles rules audit; `properties` client writes denied + server `commit`/`delete_listing` routes via Admin SDK (`lib/ingest/manual-normalize.ts`) keep the manual scan/paste→commit UX working; OAuth-refresh-token persistence re-confirmed (AES-256-GCM, server-only); `docs/architecture/auth-and-secrets.md` (+ App Check documented path). Pass 2: `parse_gmail`/`parse_raw_text` now require a verified Firebase ID token (route logic extracted to `lib/api/properties-handler.ts`; dashboard sends the token; Gmail/paste UX unchanged); route integration test `tests/properties-route.test.ts` covers auth-required/invalid-token/commit/delete/extraction gates; fresh-eyes adversarial rules re-audit found no hole and added 4 server-only deny proofs (`alert_matches`/`ingest_runs`/`provider_quota`). 42/42 rules GREEN, 202 unit GREEN, `npm run verify` GREEN. **Operator-pending (account/console, NOT code):** confirm Firebase authorized domains, Vercel env readback, GCP budget alert, App Check enablement — these gate the final live tick of "auth domains + production envs verified" but require no further code.                                                                                                                                                                   |
...
| WS17 tests/CI/release gate + production smoke                             | CLOSED `eb68b067`,`21cc24e9` (pass 1) + `__PASS2_SHA__` (pass 2). Pass 1 landed: CI workflow `.github/workflows/ci.yml` (`verify` + `rules` [Temurin JDK 21 + cached Firestore emulator] + `e2e` [Playwright chromium] jobs on push/PR to `main`; no real secrets — public Firebase web config builds clean). Playwright suite (`playwright.config.ts` + `e2e/` + `test:e2e`): 6 reachable smokes RUN+PASS, 2 OAuth-gated `test.skip` → manual runbook (no auth backdoor). 10-rung release-gate ladder (`development-workflow.md`) + production runbook (`release-runbook.md`). Pass 2 (fresh-context adversarial audit, 2026-06-10): re-verified all five pinned Action versions against the GitHub release API — bumped the one stale pin `actions/cache@v4`→`@v5` (Node-24 runner line, matching `checkout@v6`/`setup-node@v6`/`setup-java@v5`); confirmed `npm ci`, Node 24, Temurin JDK 21 (≥ emulator's JDK 11 min), emulator auto-download + no Firebase login, Playwright `--with-deps chromium`, no injected secret, not a deploy trigger. Confirmed the 6 smokes assert real behavior and the 2 skips have a legitimate live-OAuth reason with zero bypass; `INGEST_JOB_TOKEN` in `playwright.config.ts` is the labelled throwaway only. Verified the doc ladder + runbook match `package.json` scripts, the real CI jobs, and the shipped WS18 invite/accept/role behavior. Added one honest in-app Docs section (`DocsView.tsx` "Sharing a workspace": invite-by-email, viewer vs editor, accepting, the switcher, viewer read-only; Gmail-send path noted as operator-dependent) to both the TOC and content; extended `docs-toc.spec.ts` to assert the new anchor + `#sharing` heading. `npm run verify` GREEN, `npm run test:rules` 42/42 GREEN, `npm run test:e2e` 6 PASS (CMA ran for real this host) / 2 skip. **Cannot be proven until the first push:** the workflow's actual execution on GitHub-hosted runners (cache hit/miss, emulator JAR download, Playwright browser fetch). **Operator/manual residue:** first CI run on push; deployed protected-route smoke (`vercel-listings-check.ts`); live OAuth-gated UI + two-account sharing; Gmail watch/Pub-Sub + OAuth consent + authorized-domains + Vercel env readback + budget alert. |
...
- **Closed:** **WS15** — pass 1 landed (`db669711`); the pass-2 close-out audit (2026-06-10) over the post-WS18/WS16 surfaces ran and found it clean (audit-only, no code change; `npm run verify` GREEN).
- **Not started — remaining queue:**
  ...
  2. ~~**WS15 pass-2 closeout** — product-flow/metadata final audit.~~ DONE 2026-06-10 (audit-only, clean; no code change).
  3. ~~**WS17** — tests/CI/release gate + full production smoke (final gate).~~ CLOSED both passes: `eb68b067`,`21cc24e9` (pass 1) + `__PASS2_SHA__` (pass-2 adversarial audit). CI workflow + Playwright e2e suite (6 reachable smokes pass, 2 OAuth-gated documented-skip) + release-gate ladder + production runbook; pass 2 re-verified the Action pins against the GitHub release API (bumped the one stale pin `cache@v4`→`@v5`), confirmed nothing was weakened/backdoored, and added an honest in-app Docs "Sharing a workspace" section. `verify` GREEN, `test:rules` 42/42, `test:e2e` 6/8 (2 skip). **This is the final workstream — the roadmap's code lane is now CLOSED.** Operator/manual residue: first CI run on push + the live OAuth-gated / deployed-only smoke rungs.
