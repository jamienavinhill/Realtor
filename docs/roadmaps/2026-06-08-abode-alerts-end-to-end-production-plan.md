...
| WS16 auth/rules/secret hardening                                          | CODE/DOCS LANDED `155962d4` (pass 1). Full first-principles rules audit (no hole beyond the known gap); `properties` client writes denied + server `commit`/`delete_listing` routes via Admin SDK (`lib/ingest/manual-normalize.ts`) keep the manual scan/paste→commit UX working; OAuth-refresh-token persistence re-confirmed (AES-256-GCM, server-only); new `docs/architecture/auth-and-secrets.md` (+ App Check documented path). 6 new emulator cases (38/38 rules GREEN), 10 normalizer unit cases (191 unit GREEN), `npm run verify` GREEN. **Operator-pending (account/console):** confirm Firebase authorized domains, Vercel env readback, GCP budget alert, App Check enablement. Pass 2 could move `parse_gmail`/`parse_raw_text` behind ID-token auth + add a route-level integration test for commit/delete. |
