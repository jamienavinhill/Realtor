...
23. [x] **WS17** — Tests/CI/release gate and complete production smoke. _CLOSED across two passes (`eb68b067`,`21cc24e9` + pass-2 audit `120b1951`): CI workflow, Playwright e2e (6 reachable pass / 2 OAuth-gated documented-skip), 10-rung release-gate ladder + production runbook; pass-2 re-audited the CI YAML vs the GitHub release API (bumped `cache@v4`→`@v5`), tightened nothing weakened, and added an honest in-app Docs "Sharing a workspace" section. `verify` GREEN, `test:rules` 42/42, `test:e2e` 6 pass / 2 skip. Code lane closed; operator residue = first CI run on push + the live OAuth-gated/deployed-only smoke rungs._
